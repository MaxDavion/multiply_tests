...
@Test('Приложение показывает корректный результат умножения аргументов')
...
@Test('При выполнении второй операции перемножения приложение показывает новый результат')
...
@Test('Приложение отображает сообщение об ошибке если переданные аргумент не корректны')
...
def test_app_show_error_if_arguments_incorrect(page, first_value, second_value):
    ...
    assert main_page.get_error() == 'Only numbers from 0 to 100 and not empty'
